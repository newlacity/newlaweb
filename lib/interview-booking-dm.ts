...
export async function sendInterviewAcceptanceDm(params: {
  userId: string;
  username: string;
  startsAt: string;
}): Promise<boolean> {
...
export async function sendInterviewRejectionDm(params: {
  userId: string;
  username: string;
  startsAt: string;
}): Promise<boolean> {
    ...
    embeds: [
      {
        description: [
          `Bonjour **${username}**,`,
          "",
          "Nous sommes **désolés du dérangement**. Aucun membre du staff n'est **disponible** à l'horaire que vous aviez choisi.",
          "",
          "Veuillez **reprendre un nouveau créneau** sur le site pour planifier votre entretien. Merci de votre compréhension.",
        ].join("\n"),
        color: 0x006bff,
        fields: [
          {
